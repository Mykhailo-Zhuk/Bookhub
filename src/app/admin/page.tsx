...
type BookFormData = Omit<Book, "id" | "tags" | "availability" | "externalResources"> & {
  tags: string;
  availability: string;
};
...
export default function AdminPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const book: Book = {
      ...
      availability: form.availability.split(",").map((a) => a.trim()).filter(Boolean) as Availability[],
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Адмін панель</h1>
        <button
          onClick={openCreate}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {[
          ...
          <div key={stat.label} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            <p className="text-2xl font-bold text-indigo-700 dark:text-indigo-400">{stat.value}</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Складність</label>
                  <select
                    value={form.difficulty}
                    onChange={(e) => setForm((f) => ({ ...f, difficulty: e.target.value as Difficulty }))}
                    className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  >
                    {(["Початківець", "Середній", "Експерт"] as Difficulty[]).map((d) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  >
                    {(["Чернетка", "Опубліковано"] as PublishStatus[]).map((s) => (
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Короткий опис</label>
                <textarea
                  required
                  rows={2}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Повний опис</label>
                <textarea
                  rows={4}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 max-w-sm w-full text-center">
            <p className="text-gray-800 dark:text-white font-medium mb-6">Видалити цю книгу?</p>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
            {books.map((book) => (
              <tr key={book.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <td className="px-6 py-4 font-medium text-gray-900 dark:text-white max-w-xs truncate">{book.title}</td>
                <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{book.author}</td>
                <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{book.genre}</td>
                <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{book.language}</td>
                <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{book.year}</td>
